...
- **No content visibility inside TLS by default.** The default NetworkStatistics/`nettop`, `lsof`, and OS Sensor paths are metadata/process attribution paths. HTTPS Inspect Mode is a separate advanced Settings -> Developer feature for managed proxy traffic only; it uses local CA material, process-scoped trust by default, optional administrator-approved macOS System trust, active egress ToolSpan scoping before local TLS termination, redacted previews by default, metadata-only CONNECT endpoint/byte/duration evidence when decryption is unavailable or out of scope, and redacted full-payload records only when explicitly enabled.
