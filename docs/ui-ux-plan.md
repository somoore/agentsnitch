...
T6. ✅ **Shipped.** **"WHY THIS MATTERS" is the best part — promote it.** The plain-English reason string is exactly the explainability the product promises. Make it the headline of each linked card, not a row near the bottom; keep Raw Why for the machine reasons. *(`why_human` is the lead "Why this matters" value with a Raw Why toggle.)*
