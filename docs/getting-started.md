...
When HTTPS Inspect full-payload retention is enabled, Evidence Pack export does not inline retained request or response bodies. The session header includes HTTPS Inspect retention counts and warnings, and inspected event rows include local payload references only. Clearing the current UI session removes retained HTTPS Inspect payload files referenced by that session's inspected event rows.
