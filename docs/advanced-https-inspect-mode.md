...
When the daemon is running, both commands use the live daemon status snapshot so the managed proxy listener, process-scoped environment, and last inspected host reflect the active proxy rather than a static settings-only view.
